...
public class XPath extends Spider {
  ...
  public String homeContent(boolean filter) {
    ...
    if (!rule.getHomeVodNode().isEmpty()) {
      ...
      for (int i = 0; i < vodNodes.size(); i++) {
        ...
        String pic="";
        if (StringUtils.isNotEmpty(rule.getHomeVodImg())) {
          pic = vodNodes.get(i).selOne(rule.getHomeVodImg()).asString().trim();
  ...
  public String categoryContent(
    ...
    for (int i = 0; i < vodNodes.size(); i++) {
      ...
      String pic="";
      if (StringUtils.isNotEmpty(rule.getCateVodImg())) {
         pic = vodNodes.get(i).selOne(rule.getCateVodImg()).asString().trim();
    ...
    if (rule.getSearchVodNode().startsWith("json:")) {
      ...
      for (int i = 0; i < vodNodes.size(); i++) {
        ...
        if (StringUtils.isNotEmpty(pic)) {
          pic = vodNodes.get(i).selOne(rule.getSearchVodImg()).asString().trim();
          pic = rule.getSearchVodImgR(pic);
          pic = Utils.fixUrl(webUrl, pic);
        }
        String mark = "";
